...
global_prev_no80 <- prev_no80[!cause_name %in% c(causes_with_zeros)]
global_prev_no80[,unique(cause_name)] # 30 causes

# Making a list of causes to run the regression
causes <- unique(global_prev_no80$cause_id)

# I save the file here so the child script can read it
write.csv(global_prev_no80, paste0(root, 'data/intermediate/05.pre_no_80.csv'), row.names = F)
...
for (ccc in causes) {
  ...
  qsub_str <- sprintf(paste("qsub -N reg_%s -l m_mem_free=18G -l fthread=6 -P ihme_general -q all.q",
                            ...
                            "-s %s %s", sep=" "),
                      ccc, "/ihme/homes/ppedroza/Ageing_USA/ageing-/pipeline/2_analysis/06.regression_in_parallel_pre_no80.R", ccc)
  system(qsub_str)
