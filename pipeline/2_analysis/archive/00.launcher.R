...
write.csv(global_incid_no80, paste0(root, "ageing-/pipeline/running_in_parallel/data/incidence_data_no_80.csv"), row.names = F)
...
for (ccc in causes) {
  ...
  qsub_str <- sprintf(paste("qsub -N reg_%s -l m_mem_free=18G -l fthread=6 -P ihme_general -q all.q",
                            ...
                            "-s %s %s", sep=" "),
                      ccc, "/ihme/homes/ppedroza/Ageing_USA/ageing-/pipeline/running_in_parallel/01.regression_in_parallel.R", ccc)
  system(qsub_str)
...
########################################
#
# Running regression for all ages
#
#########################################

# write code here to run the regression with all adults